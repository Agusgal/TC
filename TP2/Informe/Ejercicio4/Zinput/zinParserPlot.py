...
def main():
	path = "ZinDerComp.txt" #Path del archivo de Spice con la Impendancia de entranda

	lt_parser = SpiceParser()
	data = lt_parser.parse(path, togheter=True)
	data.index.names = ["frequency"]
	data.columns = ["Impedancia MAG","Impedancia PHA"]

	#Pasamos los dB a Kohms
	data["Impedancia MAG"] = np.power(10,np.divide(data["Impedancia MAG"],20))

	#Guardamos el archivo en excel
	file_name = "PruebaZin"
