...
\end{tabular}
\caption{Ganancias de los circuitos inversores.}
\end{center}
\end{table}

Luego se procedió a realizar los gráficos de $A_{Vol}(\omega)$ para cada uno de los 3 casos.
\begin{figure}[H]	
	\centering
...
\begin{equation}
	...
	\label{noinvtransferenciaainf}
\end{equation}

Por otro lado, para $A_{Vol}$ finito, se toma lo obtenido en (\ref{equ:noinvtransferencia}), sin necesidad de aplicar cambios.

Finalmente, para incluir la consideración del polo dominante, basta con utilizar nuevamente lo establecido en (\ref{eq:PoloDom}). 

\begin{equation}
	H \left( s \right) =\frac{A_{0}}{R_3 \cdot \left(1 + s/\omega' \right)} \cdot \frac{R_3 // R_4}{1 \ + \ A_{0} \frac{R_1 // R_2}{R_2 \cdot \left(1 + s/\omega' \right)} } \cdot \left[ s \left\lbrace \omega' \cdot \left( 1 + A_{0} \frac{R_1 // R_2}{R_2 \cdot \left(1 + s/\omega' \right)} \right) \right]^{-1} \ + \ 1 \right\rbrace^{-1}
	\label{noinvtransferenciaaw}
\end{equation}
...
Por otro lado, para $A_{Vol}$ finito se considera:
...
Reemplazando con los valores presentados en la Tabla (\ref{tabla:resasignadas}), se llega a ...

\begin{center}
\textcolor{red}{\textbf{COMPLETAR REEMPLAZANDO LOS VALORES DE R}}
\textcolor{red}{\textbf{Y COMPLETAR LA TABLA QUE VIENE}}
\end{center}
...
\textcolor{red}{\textbf{MEDIME LAS COSAS ALADDIN}}
\end{center}
...
De esta forma, se observa que la transferencia ha cambiado con respecto a (\ref{equ:noinvtransferencia}) y (\ref{noinvtransferenciaainf}), pero se mantiene constante a (\ref{noinvtransferenciaaw}).
