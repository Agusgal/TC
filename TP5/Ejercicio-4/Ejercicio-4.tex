...
Como se mencionó anteriormente, los capacitores sos componentes importantes que afectan tanto el $Q$ como el $\omega_o$ del circuito. Pequeñas variaciones en ellos puede modificar la frecuencia de notch de cada etapa, haciendo que estos se acerquen o se alejen. Esto puede observarse en diagramas de Montecarlo de transferencia del sistema. Como se mencionó previamente, dado a cuestiones de disposición de componentes, se decidió presentar dicho diagrama en las Figuras (\ref{fig:mc-mod}) y (\ref{fig:mc-pha}), donde en se consideró las resistencias con una tolerancia del $1\%$ y los capacitores con una tolerancia del $5\%$, debido a que por consigna no se aceptan tolerancias mayores y dado a que los capacitores empleados fueron medidos, garantizando así el uso de componentes que cumplan con lo dicho
\begin{figure}[H]
\centering
...
De esta forma, en ciertos casos y dependiendo de dichos componentes, el factor $f_\infty$ de cada etapa puede superponerse generando que no se observe el ripple característico de la plantilla de Chebycheff. Un ejemplo de esto se presenta a continuación.
\begin{figure}[H]
\centering
...
\begin{center}
	...
	\Large{\textbf{\textcolor{red}{RANGO DINÁMICO.}}}
\end{center}
