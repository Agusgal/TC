...
Del sistema anterior, $v_a(t) = \ddot{x}(t) $, $\int v_a(t)dt = \dot{x}(t)$ y $\int \left( \int v_a(t)dt \right) dt = x(t)$ son las llamadas variables de estado. Se pueden representar estas de la forma presentada en la Figura (\ref{fig:blockKHN}).
\begin{figure}[H]
\centering
...
Como se busca confeccionar un filtro rechaza banda, se descartan los filtros TT y AM, ya que estos no poseen transferencias que sean pasa altos, por lo tanto no se puede confeccionar directamente un filtro del tipo notch. Para conseguir una transferencia como la mencionada anteriormente con una celda KHN, se requiere un amplificador operacional extra que permita restar la transferencia pasa bajos y pasa altos, mientras que para la celda FT basta con tomar los componentes adecuados. Es por ello que la celda considerada más optima para este caso es la FT.

Se desea que el filtro posea una frecuencia de notch $f_{\infty} = 37 \ kHz$, y como también se requiere que $\Delta f_a = 600 \ Hz$ y $\Delta f_p = 880 \ Hz$, utilizando
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
se obtienen $f_{p}^{-} = 36562.62 \ Hz$, $f_{p}^{+} = 37442.62 \ Hz$, $f_{a}^{-} = 36701.22 \ Hz$ y $f_{a}^{+} = 37301.22 \ Hz$. Por otro lado, para la atenuación, tanto en la banda atenuada como en la pasante, se busca $A_a=40 \ dB$ y $A_p=6 \ dB$ respectivamente. En cuanto a la ganancia, se seleccionan $0 \ dB$, mientras se debe garantizar que la profundidad de notch sea mayor o igual a $50 \ dB$. Finalmente, se requiere que la impedancia de entrada del sistema sea $Z_{in}(f) \geq 50 \ k\omega$ y que se emplee una plantilla de Chebycheff Inverso.
