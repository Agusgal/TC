...
Se desea que el filtro posea una frecuencia de notch $f_{\infty} = 37 \ kHz$, y como también se requiere que $\Delta f_a = 600 \ Hz$ y $\Delta f_p = 880 \ Hz$, se obtienen $f_{p}^{-} = 36560 \ Hz$, $f_{p}^{+} = 37440 \ Hz$, $f_{a}^{-} = 36700 \ Hz$ y $f_{a}^{+} = 37300 \ Hz$. Por otro lado, para al atenuación en cada sección, se busca $A_a=40 \ dB$ y $A_p=6 \ dB$. En cuanto a la ganancia, se seleccionan $0 \ dB$, mientras se debe garantizar que la profundidad de notch sea mayor o igual a $50 \ dB$. Finalmente, se requiere que la impedancia de entrada del sistema sea $Z_{in}(f) \geq 50 \ k\omega$ y que se emplee una plantilla de Chebycheff Inverso.
